...
public class OauthHelper {
    ...
    private static volatile HttpClient tokenClient = null;
    private static volatile HttpClient introspectionClient = null;
    private static volatile HttpClient signClient = null;
    private static volatile HttpClient derefClient = null;

    /**
     * Get an access token from the token service. A Result of TokenResponse will be
     * returned if the invocation is successfully.
     * Otherwise, a Result of Status will be returned.
     *
     * @param tokenRequest token request constructed from the client.yml token
     *                     section.
     ...
     * @param tokenRequest token request constructed from the client.yml token
     *                     section.
     * @param envTag       the environment tag from the server.yml for service
     *                     lookup.
     * @return Result of TokenResponse or error Status.
     */
    public static Result<TokenResponse> getTokenResult(TokenRequest tokenRequest, String envTag) {
        ...
        if (tokenClient == null || clientConfig != ClientConfig.get()) {
            synchronized (OauthHelper.class) {
                if (tokenClient == null || clientConfig != ClientConfig.get()) {
                    ...
                    try {
                        ...
                        if (logger.isTraceEnabled())
                            logger.trace("proxyHost = " + tokenRequest.getProxyHost() + " proxyPort = "
                                    + tokenRequest.getProxyPort());
                        if (!StringUtils.isBlank(tokenRequest.getProxyHost()))
                            clientBuilder.proxy(ProxySelector.of(new InetSocketAddress(tokenRequest.getProxyHost(),
                                    tokenRequest.getProxyPort() == 0 ? 443 : tokenRequest.getProxyPort())));
                        ...
                        if (tlsMap == null || tlsMap.get(TLSConfig.VERIFY_HOSTNAME) == null || !Boolean.TRUE.equals(
                                Config.loadBooleanValue(TLSConfig.VERIFY_HOSTNAME,
                                        tlsMap.get(TLSConfig.VERIFY_HOSTNAME)))) {
        ...
        try {
            ...
            if (tokenRequest.getServerUrl() == null) {
                if (logger.isTraceEnabled())
                    logger.trace("serviceUrl is empty and could not discovery serviceUrl. tokenRequest = "
            ...
            CompletableFuture<HttpResponse<String>> response = tokenClient.sendAsync(request,
    ...
    /**
     * Get a signed JWT token from token service to ensure that nobody can modify
     * the payload when the token
     * is passed from service to service. Unlike the access JWT token, this token is
     * ensure the data integrity
     * with signature.
     *
     * @param signRequest SignRequest that is constructed from the client.yml sign
     *                    section
     ...
     * @param signRequest SignRequest that is constructed from the client.yml sign
     *                    section
     * @param envTag      environment tag that is used for service lookup if
     *                    serviceId is used.
     * @return Result that contains TokenResponse or error status when failed.
     */
    public static Result<TokenResponse> getSignResult(SignRequest signRequest, String envTag) {
        if (signClient == null || clientConfig != ClientConfig.get()) {
            synchronized (OauthHelper.class) {
                if (signClient == null || clientConfig != ClientConfig.get()) {
                    clientConfig = ClientConfig.get();
                    tokenClient = null;
                    ...
                    try {
                        ...
                        if (signRequest.getProxyHost() != null)
                            clientBuilder.proxy(ProxySelector.of(new InetSocketAddress(signRequest.getProxyHost(),
                                    signRequest.getProxyPort() == 0 ? 443 : signRequest.getProxyPort())));
        ...
        try {
            ...
            if (signRequest.getClientId() != null && signRequest.getClientSecret() != null) {
                requestBuilder.setHeader(Headers.AUTHORIZATION_STRING,
            ...
            CompletableFuture<HttpResponse<String>> response = signClient.sendAsync(request,
                    HttpResponse.BodyHandlers.ofString());
            String body = response.thenApply(HttpResponse::body).get();
            HttpHeaders headers = response.thenApply(HttpResponse::headers).get();
            return handleResponse(getContentTypeHeaders(headers), body);
    ...
    /**
     * Get an access token from the token service based on a SAML token request. A
     * Result of TokenResponse will be returned
     * if the invocation is successfully. Otherwise, a Result of Status will be
     * returned.
     *
     * @param tokenRequest token request constructed from the client.yml token
     *                     section.
    ...
    /**
     * Get an access token from the token service based on a token exchange request.
     * A Result of TokenResponse will be returned
     * if the invocation is successfully. Otherwise, a Result of Status will be
     * returned.
     *
     * @param tokenRequest token request constructed from the client.yml token
    ...
    public static Result<TokenResponse> getTokenFromTokenExchangeResult(TokenExchangeRequest tokenRequest,
        ...
        try {
            ...
            IClientRequestComposable requestComposer = ClientRequestComposerProvider.getInstance()
                    .getComposer(ClientRequestComposerProvider.ClientRequestComposers.TOKEN_EXCHANGE_REQUEST_COMPOSER);
            final HttpRequest request = requestComposer.composeClientRequest(tokenRequest);
            CompletableFuture<HttpResponse<String>> response = tokenClient.sendAsync(request,
                    HttpResponse.BodyHandlers.ofString());
            String body = response.thenApply(HttpResponse::body).get();
    ...
    /**
     * Get the certificate from key distribution service of OAuth 2.0 provider with
     * the kid.
     *
     * @param keyRequest One of the sub classes to get the key for access token or
     *                   sign token.
     * @return String of the certificate
     * @throws ClientException throw exception if communication with the service
     *                         fails.
     */
    ...
    /**
     * Get the token info from the introspection endpoint of OAuth 2.0 provider with
     * the swt.
     * 
     * @param token                The simple web token that needs to be
     *                             introspected.
     ...
     *                         fails.
     */
    public static Result<String> getIntrospection(String token, IntrospectionRequest introspectionRequest)
     ...
     * @param keyRequest One of the sub classes to get the key for access token or
     *                   sign token.
     * @param envTag     the environment tag from the server.yml for the cluster
     *                   lookup.
     * @return String of the certificate
     * @throws ClientException throw exception if communication with the service
     *                         fails.
    ...
    public static String getKey(KeyRequest keyRequest, String envTag) throws ClientException {
        ...
        try {
            ...
            if (!StringUtils.isBlank(keyRequest.getProxyHost()))
                clientBuilder.proxy(ProxySelector.of(new InetSocketAddress(keyRequest.getProxyHost(),
                        keyRequest.getProxyPort() == 0 ? 443 : keyRequest.getProxyPort())));
            ...
            if (tlsMap == null || tlsMap.get(TLSConfig.VERIFY_HOSTNAME) == null || !Boolean.TRUE.equals(
                    Config.loadBooleanValue(TLSConfig.VERIFY_HOSTNAME, tlsMap.get(TLSConfig.VERIFY_HOSTNAME)))) {
                final Properties props = System.getProperties();
    ...
    /**
     * Get the introspection of the simple web token from OAuth 2.0 provider with
     * the swt.
     * 
     * @param token                The simple web token to be introspected.
     * @param introspectionRequest One of the subclasses to get the introspection
     *                             for access token.
     * @param envTag               the environment tag from the server.yml for the
     *                             cluster lookup.
     * @return String of the token info in JSON
     * @throws ClientException throw exception if communication with the service
     *                         fails.
     */
    public static Result<String> getIntrospection(String token, IntrospectionRequest introspectionRequest,
            String envTag) throws ClientException {
        if (introspectionClient == null || clientConfig != ClientConfig.get()) {
            synchronized (OauthHelper.class) {
                if (introspectionClient == null || clientConfig != ClientConfig.get()) {
                    clientConfig = ClientConfig.get();
                    tokenClient = null;
                    ...
                    try {
                        HttpClient.Builder clientBuilder = HttpClient.newBuilder()
                                ...
                                .sslContext(HttpClientRequest.createSSLContext());
                        if (logger.isTraceEnabled())
                            logger.trace("proxyHost = " + introspectionRequest.getProxyHost() + " proxyPort = "
                                    + introspectionRequest.getProxyPort());
                        if (!StringUtils.isBlank(introspectionRequest.getProxyHost()))
                            clientBuilder
                                    .proxy(ProxySelector.of(new InetSocketAddress(introspectionRequest.getProxyHost(),
                                            introspectionRequest.getProxyPort() == 0 ? 443
                                                    : introspectionRequest.getProxyPort())));
        ...
        if (logger.isTraceEnabled())
            logger.trace("introspection service url = " + serverUrl);
        try {
            ...
            if (logger.isTraceEnabled())
                logger.trace("form = " + form + " url = " + serverUrl + introspectionRequest.getUri() + " clientId = "
                        + introspectionRequest.getClientId() + " clientSecret = "
                        + introspectionRequest.getClientSecret());
            HttpRequest.Builder requestBuilder = HttpRequest.newBuilder()
                    .timeout(Duration.ofMillis(ClientConfig.get().getRequest().getTimeout()))
            ...
            if (introspectionRequest.getClientId() != null && introspectionRequest.getClientSecret() != null) {
                requestBuilder.setHeader(Headers.AUTHORIZATION_STRING,
                        getBasicAuthHeader(introspectionRequest.getClientId(), introspectionRequest.getClientSecret()));
            }
            requestBuilder.setHeader(Headers.CONTENT_TYPE_STRING, APPLICATION_FORM_URLENCODED_VALUE);

            HttpRequest request = requestBuilder.build();

            CompletableFuture<HttpResponse<String>> response = introspectionClient.sendAsync(request,
                    HttpResponse.BodyHandlers.ofString());
            String body = response.thenApply(HttpResponse::body).get();
            if (logger.isTraceEnabled())
    ...
    /**
     * De-reference a simple web token to JWT token from OAuth 2.0 provider. This is
     * normally called from the light-router.
     *
     * @param derefRequest a DerefRequest object that is constructed from the
     *                     client.yml file.
     ...
     * @param derefRequest a DerefRequest object that is constructed from the
     *                     client.yml file.
     * @param envTag       an environment tag from the server.yml for cluster
     *                     service lookup.
    ...
    public static String derefToken(DerefRequest derefRequest, String envTag) throws ClientException {
        if (derefClient == null || clientConfig != ClientConfig.get()) {
            synchronized (OauthHelper.class) {
                if (derefClient == null || clientConfig != ClientConfig.get()) {
                    clientConfig = ClientConfig.get();
                    tokenClient = null;
                    ...
                    try {
                        ...
                        if (derefRequest.getProxyHost() != null)
                            clientBuilder.proxy(ProxySelector.of(new InetSocketAddress(derefRequest.getProxyHost(),
                                    derefRequest.getProxyPort() == 0 ? 443 : derefRequest.getProxyPort())));
        ...
        try {
            ...
            CompletableFuture<HttpResponse<String>> response = derefClient.sendAsync(request,
                    HttpResponse.BodyHandlers.ofString());
            return response.thenApply(HttpResponse::body).get(ClientConfig.get().getRequest().getTimeout(),
                    TimeUnit.MILLISECONDS);
        } catch (Exception e) {
    ...
    public static String encodeCredentials(String clientId, String clientSecret) {
        ...
        String encodedValue;
        byte[] encodedBytes = encodeBase64(cred.getBytes(UTF_8));
    ...
    public static String getEncodedString(TokenRequest request) throws UnsupportedEncodingException {
        ...
        if (ClientConfig.AUTHORIZATION_CODE.equals(request.getGrantType())) {
            ...
            // The redirectUri can be null so that OAuth 2.0 provider will use the
            // redirectUri defined in the client registration
    ...
    private static Result<TokenResponse> handleResponse(ContentType contentType, String responseBody) {
        ...
        if (logger.isTraceEnabled())
            logger.trace("contentType = {} responseBody = {}", contentType, responseBody);
        // only accept json format response so that can map to a TokenResponse,
        // otherwise escapes server's response and return to the client.
        ...
        if (responseBody != null && !responseBody.isEmpty()) {
            ...
            // sometimes, the token response contains an error status instead of the access
            // token.
            if (tokenResponse != null && tokenResponse.getAccessToken() != null) {
                result = Success.of(tokenResponse);
            } else {
                result = Failure.of(new Status(tokenResponse.getStatusCode(), tokenResponse.getCode(),
                        tokenResponse.getMessage(), tokenResponse.getDescription(), tokenResponse.getSeverity()));
            }
        } else {
     ...
     * based on the expire time of the jwt, to determine if need to renew jwt or
     * not.
     * to avoid modifying class member which will case thread-safe problem, move
     * this method from Http2Client to this helper class.
     * 
    ...
    public static Result<Jwt> populateCCToken(final Jwt jwt) {
        ...
        // the same jwt shouldn't be renew at the same time. different jwt shouldn't
        // affect each other's renew activity.
        synchronized (jwt) {
     ...
     * @return Jwt when success, it will be the same object as the jwt you passed
     *         in; return Status when fail;
     */
    private static Result<Jwt> renewCCTokenSync(final Jwt jwt) {
        // Already expired, try to renew getCCTokenSynchronously but let requests use
        // the old token.
        logger.trace("In renew window and token is already expired.");
        // the token can be renewed when it's not on renewing or current time is lager
        // than retrying interval
     ...
     * When fail, it will swallow the exception, so no need return type to be
     * handled by caller.
     * 
     * @param jwt the jwt you want to renew
     */
    private static void renewCCTokenAsync(final Jwt jwt) {
        ...
        if (!jwt.isRenewing() || System.currentTimeMillis() > jwt.getEarlyRetryTimeout()) {
            ...
            executor.schedule(() -> {
                ...
                // set renewing flag to false after response, doesn't matter if it's success or
                // fail.
                jwt.setRenewing(false);
    ...
    private static Result<Jwt> getCCTokenRemotely(final Jwt jwt) {
        TokenRequest tokenRequest = new ClientCredentialsRequest(jwt.getAuthServerConfig());
    ...
    protected static Result<Jwt> updateJwtWithTokenResponse(Jwt jwt, TokenResponse tokenResponse) {
        ...
        // extracting the jwt.exp claim as it is the most reliable source to calculate
        // expire time.
        // Auth Server response payload body is not as reliable since it can be cached
        // and not updated.
        try {
        ...
        logger.info("Get client credentials token {} with exp claim {} milliseconds", jwt.getJwt().substring(0, 20),
                jwt.getExpire());
    ...
    public static long getJwtExp(String jwt) throws JsonProcessingException {
        ...
        // String header = new String(Base64.decodeBase64(chunks[0])); // need to decode
        // header?
     ...
     * otherwise remains the default scope value which already inside tokenRequest
     * when create ClientCredentialsRequest;
     * 
    ...
    private static void setScope(TokenRequest tokenRequest, Jwt jwt) {
        if (jwt.getKey() != null && jwt.getKey().getScopes() != null && !jwt.getKey().getScopes().isEmpty()) {
            tokenRequest.setScope(new ArrayList<>() {
                {
                    addAll(jwt.getKey().getScopesSet());
                }
            });
        }
    }
     ...
     * it should be used in very rare cases because the server should not return xml
     * format message
     * 
     * @param nonEscapedXmlStr String
     * @return String
     */
